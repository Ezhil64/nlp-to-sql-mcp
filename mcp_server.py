# mcp_server.py
# FastMCP PostgreSQL MCP server (SSE) exposing tools: run_query, get_tables.
# Start multiple instances on different ports for replication.
# pip install fastmcp psycopg2-binary pandas
...
from fastmcp import FastMCP  # FastMCP server
...
def is_select_only(sql_text: str) -> bool:
    ...
    if re.search(r"\b(INSERT|UPDATE|DELETE|DROP|ALTER|TRUNCATE|CREATE|REPLACE|GRANT|REVOKE|VACUUM|ANALYZE|COMMENT|MERGE)\b", s, flags=re.IGNORECASE):
        return False
    return True
...
def get_connection(read_only: bool = True):
    ...
    conn = psycopg2.connect(host=DB_HOST, port=DB_PORT, dbname=DB_NAME, user=DB_USER, password=DB_PASSWORD, options=opts)
...
def run_sql(sql_text: str, allow_writes: bool) -> Dict[str, Any]:
    if is_dangerous_sql(sql_text): raise ValueError("Extremely dangerous SQL detected.")
    if not allow_writes and not is_select_only(sql_text): raise ValueError("Read-only mode: only SELECT/WITH are allowed.")
    ...
    with get_connection(read_only=not allow_writes) as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            if cur.description:
                ...
                return {"type":"select","columns":cols,"rows":[dict(r) for r in rows],"row_count":len(rows)}
            return {"type":"write","affected_rows":cur.rowcount,"message":f"OK, {cur.rowcount} row(s) affected."}
...
def seed_employees_if_empty(table_name: Optional[str] = None) -> None:
    ...
    with get_connection(read_only=False) as conn:
        with conn.cursor() as cur:
            ...
            insert_sql = psql.SQL("INSERT INTO {} (name, salary, age) VALUES (%s,%s,%s)").format(qualified_table(schema, table))
...
@mcp.tool(name="get_tables", description="Return {schema: {table: [columns...]}} for configured schema.")
...
if __name__ == "__main__":
    ...
    mcp.run(transport="sse", host=MCP_HOST, port=MCP_PORT)  # SSE transport
